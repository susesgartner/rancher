...
func (r *ClusterTemplateTestSuite) TestProvisionK3STemplateCluster() {
	_, err := steve.CreateAndWaitForResourceState(r.client, localCluster, stevetypes.ClusterRepo, r.templateConfig.Repo, stevestates.Active, 5*time.Second, defaults.FiveMinuteTimeout)
	require.NoError(r.T(), err)
